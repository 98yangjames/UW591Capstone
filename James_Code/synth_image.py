...
def synth_image():
    ...
    cv2.imwrite(text_corpus + " fake_fusion.jpg", fake_fusion)
    cv2.imwrite(text_corpus + " fake_text.jpg", fake_text)
    cv2.imwrite(text_corpus + " fake_bg.jpg", fake_bg)

def synth_image_head(style_image, text_corpus, name):
    args = ArgsParser().parse_args()
    image_synthesiser = ImageSynthesiser()
    style_image_path = style_image #examples/style_images/1_10_22_download.jpg
    img = cv2.imread(style_image_path)
    text_corpus = text_corpus #HumanInsulin/SolubleInsulin
    language = args.language
    ...
    fake_text = synth_result["fake_text"]
    fake_bg = synth_result["fake_bg"]
    cv2.imwrite(name + " fake_fusion.jpg", fake_fusion)
    cv2.imwrite(name + " fake_text.jpg", fake_text)
    cv2.imwrite(name + " fake_bg.jpg", fake_bg)
...
#         i += 1
#         print(i*100 / len(df['SaltName'].unique()))
#         synth_image("examples/style_images/1_10_22_download" +str(i) + ".jpg", name, str(name) + "examples/style_images/1_10_22_download" +str(i) + ".jpg")
# main()